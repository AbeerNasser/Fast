        ...
        <div class="section-body">
            <div class="container-fluid">
                <div class="row clearfix">
                    <div class="col-lg-12">
                       <div class="mb-2">
                           <form class="form-inline my-3" method="post" action="{{url('support')}}">
                               @csrf
                               @method('post')
                                                ...
                                                <td>{{$support->phone}}</td>
                                                 ...
                                                 <td>
                                                    ...
                                                    @if($support->type_of_problem == 'العميل الغي الطلب')
                                                        <form style="display:inline-table !important;" action="{{url('deleteOrder/'.$support->id)}}" method="post">
                                                            @csrf
                                                            @method('post')
                                                    ...
                                                    @if($support->type_of_problem =='تاخير المندوب')
                                                        <form style="display:inline-table !important;" action="{{url('delayOrder/'.$support->id)}}" method="post">
                                                            @csrf
                                                            @method('post')
                                                            <button  type="submit" class="btn btn-info">اعاده ارسال </button>
                                                        </form>
                                                    @endif
                                                    <form style="display:inline-table !important;" action="{{url('activeSupport/'.$support->id)}}" method="post">
                                                        @csrf
                                                        @method('post')
